...
def main():
    ...
    with tab2:
        if 'gameOption' not in st.session_state:
            st.session_state.gameOption = ''

        gameOption = st.selectbox("Choose option in Games site",['', 'Create Game', 'Search Game'])
        if gameOption == 'Create Game':
