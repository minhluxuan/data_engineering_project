...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(['Event Result View', 'Event Result Creating', 'Event Result Updating', 'Event Result Deleting', ])
    with tab1:
        st.header("Searching the result of each athlete")
        st.write("Please look up the edition, country noc, result and athlete IDs before searching (empty = all).")
        
        # Tạo 4 hộp xổ xuống
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            edition_id = st.text_input("Edition Game ID:", placeholder="Enter number edition ID")
        with col2:
            noc = st.text_input("NOC of country:", placeholder="Enter NOC")
        with col3:
            result_id = st.text_input("Result ID:", placeholder="Enter number result ID")
        with col4:
            athlete_id = st.text_input("Athlete ID:", placeholder="Enter number athlete ID")
        
        if st.button("Confirm Input"):
            if edition_id == "": edition_id = 9132662681
            if noc == "": noc = 'All'
            if result_id == "": result_id = 9132662681
            if athlete_id == "": athlete_id = 9132662681
        
        response = EventResultOperation.search(edition_id, noc, result_id, athlete_id)
    ...
    with tab2:
            ...
            st.write("Please look up the edition, country noc, result and athlete IDs before creating.")
            ...
            with st.expander("Create new event result", expanded=True):
                with st.form(key='post_form'):
                    ...
                    isTeamSport = st.radio(label='Is Team Sport?', options=['Yes', 'No'])
                    medal = st.radio(label='Medal', options=['Gold', 'Silver', 'Bronze', 'None'])
                    submit_button = st.form_submit_button(label='Submit')

                    if submit_button:
                        if not edition_id or not noc or not result_id or not athlete_id:
                            st.write("Edition ID, Country NOC, Result ID, Athelete ID cannot be empty, please enter all values.")
                        else:
                            isTeamSport = True if isTeamSport == 'Yes' else False
                            if medal == 'None': medal = None
    ...
    with tab3:
            ...
            st.write("Please look up the edition, country noc, result and athlete IDs before creating. (Not be empty)")
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                edition_id = st.text_input("Edition Game ID:", placeholder="Enter number edition ID", key = 'u0')
            with col2:
                noc = st.text_input("NOC of country:", placeholder="Enter NOC", key = 'u1')
            with col3:
                result_id = st.text_input("Result ID:", placeholder="Enter number result ID", key = 'u2')
            with col4:
                athlete_id = st.text_input("Athlete ID:", placeholder="Enter number athlete ID", key = 'u3')
            ...
            if not edition_id or not noc or not result_id or not athlete_id:
                ...
                if response is not None and response.status_code == 200:
                    data = response.json()
                    if isinstance(data, list) and data:  # Kiểm tra nếu dữ liệu là list và không rỗng
                        ...
                        st.write("Please update the information below. Leave blank if no changes are needed.")
                        with st.form(key='update_form'):
                            edition_id1 = st.text_input("Edition ID", value=old_data.get("edition_id", ""))
                            country_noc1 = st.text_input("Country NOC", value=old_data.get("country_noc", ""))
                            sport1 = st.text_input("Sport", value=old_data.get("sport", ""))
                            event1 = st.text_input("Event", value=old_data.get("event", ""))
                            result_id1 = st.text_input("Result ID", value=old_data.get("result_id", ""))
                            athlete_id1 = st.text_input("Athlete ID", value=old_data.get("athlete_id", ""))
                            pos1 = st.text_input("Position", value=old_data.get("pos", ""))
                            isTeamSport1 = st.radio("Is Team Sport?", options=["Yes", "No"], index=0 if old_data.get("isTeamSport") else 1)
                            medal1 = st.radio("Medal", options=["Gold", "Silver", "Bronze", "None"], index={"Gold": 0, "Silver": 1, "Bronze": 2, None: 3}[old_data.get("medal")])
                        ...
                        if submit_button1:
                            ...
                            if updated_data:
                                response = EventResultOperation.update(edition_id, noc, result_id, athlete_id, updated_data)
                                if response.status_code == 200:
                                    st.success(response.json()['message'])
                                else:
                                    st.write(f"An error occurred: {response.status_code}")
    ...
    with tab4:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                edition_id = st.text_input("Edition Game ID:", placeholder="Enter number edition ID", key = 'u4')
            with col2:
                noc = st.text_input("NOC of country:", placeholder="Enter NOC", key = 'u5')
            with col3:
                result_id = st.text_input("Result ID:", placeholder="Enter number result ID", key = 'u6')
            with col4:
                athlete_id = st.text_input("Athlete ID:", placeholder="Enter number athlete ID", key = 'u7')
            ...
            if not edition_id or not noc or not result_id or not athlete_id:
                ...
                if response is not None and response.status_code == 200:
                    ...
                    if st.button("Confirm Delete"):
                        response = EventResultOperation.delete(edition_id, noc, result_id, athlete_id)
